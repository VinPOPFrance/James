...
export const metadata: Metadata = {
  ...
  alternates: {
    ...
    languages: {
      ...
      "x-default": `${siteConfig.siteUrl}/12-week-body-reset-program`,
    },
  },
};
...
export default function ProgramPage() {
  ...
  return (
    <PageLayout>
      <Reveal>
        <section className="rounded-section border border-copper/30 bg-white px-7 py-10 md:px-14 md:py-14">
        ...
        <section className="rounded-section border border-hairline bg-white px-8 py-14 md:px-12">
          <div className="mx-auto max-w-5xl">
            ...
            <div className="mb-8 grid items-center gap-6 md:grid-cols-[1.15fr_0.85fr] md:gap-8">
              <div className="space-y-3 md:ml-auto md:max-w-[34rem] md:pl-6">
                {t.included.items.map((item, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <icons.check size={16} className="mt-0.5 shrink-0 text-sage" />
                    <span className="text-[17px] text-inkSoft">{item}</span>
                  </div>
                ))}
              </div>
